...
export type ApiResponse<T> = {
  data: T | null
  error: openapi.models.StandardResponseSchema | null
...
export const createApiResponse = (message: string, status: number, field?: string): ApiResponse<openapi.models.StandardResponseSchema> => {
  const response: openapi.models.StandardResponseSchema = {
    message: message,
    validation: field ? [{ path: field, message: `${field} is a required field` }] : undefined,
    status: String(status),
  }

  return new ApiResponseWrapper<openapi.models.StandardResponseSchema>(response, status, location?.href).unwrap()
}

export class ApiResponseWrapper<T> {
  url: string
  data: T | openapi.models.StandardResponseSchema
  errors: ValidationErrors
  headers: Record<string, string> = {}
  serverError: string | null = null
  status: number

  constructor(data: T | openapi.models.StandardResponseSchema, status: number, url: string, responseHeaders?: Headers) {
    this.data = data
    this.status = status
  ...
  unwrap(): ApiResponse<T> {
    return {
      ...
      error: this.success() ? null : this.data as openapi.models.StandardResponseSchema,
      errors: this.errors,
  ...
  error(): this is { data: openapi.models.StandardResponseSchema } {
    return this.status >= 500
  }

  validation(): this is { data: openapi.models.StandardResponseSchema } {
    return this.status >= 400 && this.status < 500
  }

  conflict(): this is { data: openapi.models.StandardResponseSchema } {
