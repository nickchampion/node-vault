...
export const createResendClient = () => {
  ...
  return {
    ...
    render: async (path: string, params: Record<string, string>, headers?: Record<string, string>): Promise<string> => {
      const url = `${serverConfiguration.app}${path}?${new URLSearchParams(params)}`
